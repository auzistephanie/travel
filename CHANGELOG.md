...
## 8y. Identity 重構：抽 resolver + useTripIdentity（2026-07-11）
- **背景**：8w 審視技術債 #1——身份四套來源（auth／URL `?m=`／localStorage／揀名）+ `manualOverride` 散落 TripShell 嘅 initial state、兩個 effect 同一個 handler，冇單一出處。方案（報告第三節）批准後執行；`manualOverride` 升級方案 Stephanie 揀咗「寫入 sessionStorage」。
- **新結構**：
  - `src/lib/identityResolver.ts`（純函數 +8 unit test）：全套優先次序一個 function 講晒——auth（除非 override）→ URL → localStorage → none；**url/storage 嘅 member id 而家會驗證真係存在於 members 先信**（以前盲信，member 被刪後條舊連結會入到幽靈身份——順手修咗）。
  - `src/hooks/useTripIdentity.ts`：包住 resolver + 全部副作用（auth 訂閱、身份寫返 URL/localStorage、owner 首次登入自動綁定、切換身份）。用戶喺揀名畫面明確揀嘅身份（`manual` source）最高優先，auth 唔會搶。
  - `TripShell.tsx` 淨返「攞 identity、揀 render 邊個畫面」，identity 相關 state/effect 全部搬走。
- **行為升級（8l 已知限制修咗）**：「切換身份」嘅 override 由 in-memory 改寫入 sessionStorage（經 safeStorage，key `whoami-override:{shareCode}`）——同一 tab session 內 reload 都記得你揀咗第二個身份，唔會俾已綁定 Google 帳戶搶返做 owner；閂 tab 重開先回復 auto 識別。

...
export function AddFlightModal({ members, onAdd, onClose }: AddFlightModalProps) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <input id="flight-date" type="date" value={date} onChange={(e) => setDate(e.target.value)} required />

      <button type="button" onClick={handleLookup} disabled={looking || !code || !date}>
        查詢
      </button>
      {lookupHint && <p>{lookupHint}</p>}

      <label htmlFor="flight-from-airport">出發機場</label>
      <input id="flight-from-airport" value={fromAirport} onChange={(e) => setFromAirport(e.target.value)} required />

      <label htmlFor="flight-to-airport">到達機場</label>
      <input id="flight-to-airport" value={toAirport} onChange={(e) => setToAirport(e.target.value)} required />

      <label htmlFor="flight-from-time">出發時間</label>
      <input id="flight-from-time" value={fromTime} onChange={(e) => setFromTime(e.target.value)} required />

      <label htmlFor="flight-to-time">到達時間</label>
      <input id="flight-to-time" value={toTime} onChange={(e) => setToTime(e.target.value)} required />

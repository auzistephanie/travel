...
5. 設定環境變數：
   ...
   `VITE_AVIATIONSTACK_KEY`（航班自動查詢）、`VITE_GOOGLE_MAPS_KEY`（地圖/天氣周邊/交通連接）同
   ...
   - Google Maps key：[Google Cloud Console](https://console.cloud.google.com) 開 project，
     啟用 **Places API (New)** 同 **Routes API**，建立 API key（記得設 HTTP referrer 限制）。
...
3. 喺 Vercel project 嘅 **Settings → Environment Variables**，填入同 `.env` 一樣嘅變數
   ...
   `VITE_GOOGLE_MAPS_KEY` / `VITE_TAGGUN_API_KEY`）。
